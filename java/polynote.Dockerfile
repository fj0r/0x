...
WORKDIR /opt
RUN set -eux \
  ; spark_version=$(xh $github_api/${spark_repo}/releases $github_header | jq -r '.[0].tag_name'|cut -c 2-) \
  ; spark_version=3.1.2 \
  ; pip3 install \
      jep \
      jedi \
      ...
      numpy \
      pandas \
  ; xh -F http://apache.claz.org/spark/spark-${spark_version}/spark-${spark_version}-bin-hadoop3.2.tgz | tar -xzvpf - \
  ; mv spark* /opt/spark \
  ; polynote_version=$(xh $github_api/${polynote_repo}/releases $github_header | jq -r '.[0].tag_name') \
  ; xh -F https://github.com/polynote/polynote/releases/download/${polynote_version}/polynote-dist.tar.gz | tar -xzvpf -
