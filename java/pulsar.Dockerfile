...
WORKDIR /pulsar
RUN set -eux \
  ; pulsar_version=$(curl --retry 3 -fsSL https://api.github.com/repos/apache/pulsar/releases -H "Accept: application/vnd.github.v3+json" | jq -r '.[0].tag_name'|cut -c 2-) \
  ; curl --retry 3 -fsSL https://github.com/apache/pulsar/archive/refs/tags/v${pulsar_version}.tar.gz | tar zxf - --strip-components=1 \
